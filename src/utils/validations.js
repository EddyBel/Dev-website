...
export function validateValues(...values) {
  ...
  for (let value of values) if (value === null || value === undefined) return false;
 ...
 * @returns {boolean} Resultado de la validación
 */
export function validateArrays(...arrays) {
  // Recorremos todos los argumentos con un bucle for...of
  // Validamos si el argumento es array y su tamaño es mas grande que 0
  // En caso de que no retorna un false
  for (let array of arrays) if (!Array.isArray(array) || array?.length === 0) return false;
